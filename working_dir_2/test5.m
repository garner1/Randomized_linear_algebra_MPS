...
%         [Uh,Vh] = gaugeH(Uh,Vh,[],[]);
%%
        auxn = reshape(permute(contracttensors(conj(mps{site}),3,3,mps{site},3,3),[1,3,2,4]),[D*D,D*D]);
        Pn = reshape(Un,[D*1*D,pn])*reshape(Vn,[pn,D*1*D]);
        aux = contracttensors(mpo{site},4,4,mps{site},3,3);
        aux = contracttensors(aux,5,[3],conj(mps{site}),3,[3]);
        aux = reshape(permute(aux,[5,1,3,6,2,4]),[D*5*D,D*5*D]);
        Ph = reshape(Uh,[D*5*D,ph])*reshape(Vh,[ph,D*5*D]);
        [pnn,~,errorn] = exact(mpoid,mps,Un,Vn,site,D,pn);
        [phh,~,errorh] = exact(mpo,mps,Uh,Vh,site,D,ph);
%%
...
%         disp(real(nn2));
        disp([errorh,errorn]);
        disp([abs(trace((Ph-phh)*aux)/trace(phh*aux)),abs(trace((Pn-pnn)*auxn)/trace(pnn*auxn))]);
        disp('---')
