...
w=expm(-1i*dt*h);  %[d^2,d^2]~[(s1,s2);(s1',s2')]
...
mps0=cell(1,N);
for j=1:N
    if j<N/2, state=[0; 1]; else state=[1; 0]; end
...
% mps_in = mps;
mps = mps_in; 
% mzvalues1=[]; factorization1 = 'svd'; 
mzvalues3=[]; factorization1 = 'fastqr';
% tempo1 = [];
tic
for step=1:10
%     fprintf('Step %2d: ',step); 
    [mps]=reduceD(mps,mpo_even,D,factorization1,'qr'); 
    [mps]=reduceD(mps,mpo_odd,D,factorization1,'qr'); 
    mz=real(expectationvalue(mps,oset)); 
%     mzvalues1=[mzvalues1,mz]; 
    mzvalues3=[mzvalues3,mz]; 
%     fprintf('mz=%g\n',mz);
%     plot(mzvalues1(1:step));drawnow;
    plot(1:step,mzvalues1(1:step),1:step,mzvalues3(1:step));drawnow;
end
toc
% plot(real(mzvalues))
%     [mps,tempo1]=reduceD(mps,mpo_even,D,factorization,tempo1);
%     [mps,tempo1]=reduceD(mps,mpo_odd,D,factorization,tempo1);
