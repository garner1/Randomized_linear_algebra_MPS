function [U,S,V] = svd_fast(A,k,its,l)
%PCA  Low-rank approximation in SVD form.
%
%
%   [U,S,V] = svd_fast(A)  constructs a nearly optimal rank-6 approximation
...
%
%   [U,S,V] = svd_fast(A,k)  constructs a nearly optimal rank-k approximation
%             USV' to A, using 2 full iterations of a block Lanczos method
%             of block size k+2, started with an n x (k+2) random matrix,
%             when A is m x n; the ref. below explains "nearly optimal."
%             k must be a positive integer <= the smallest dimension of A.
%
%   [U,S,V] = svd_fast(A,k,its)  constructs a nearly optimal rank-k approx. USV'
...
%
%   [U,S,V] = svd_fast(A,k,its,l)  constructs a nearly optimal rank-k approx.
...
%
%
%   Note: it invokes RAND. To obtain repeatable results,
